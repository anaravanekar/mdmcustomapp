...
//		writer.addJS("console.log('update assingment successful');");
		...
		writer.addJS("var stateSelect = document.getElementById(\"customStateSelect\"); var provinceSelect = document.getElementById(\"customProvinceSelect\"); var blankOption = new Option(\"[not defined]\", \"\"); blankOption.setAttribute(\"disabled\", \"\"); blankOption.setAttribute(\"style\", \"display:none;\"); if (territoryTypeMap[countryCode] === \"STATE\") { urlForOptions = urlForOptions + \"state/\" + countryCode; console.log('is state1'); toggeleStandardFields(false); } else if (territoryTypeMap[countryCode] === \"PROVINCE\") { console.log('is province1'); urlForOptions = urlForOptions + \"province/\" + countryCode; toggeleStandardFields(false); } else { console.log('is none1'); while (provinceSelect.options.length) { provinceSelect.remove(0); } provinceSelect.options.add(blankOption); while (stateSelect.options.length) { stateSelect.remove(0); } stateSelect.options.add(blankOption); ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, null); toggeleStandardFields(true); try { ebx_form_setValue(statePrefixedPath, currentStateValue); ebx_form_setValue(provincePrefixedPath, currentProvinceValue); } catch (err) {} return; } var xhr = new XMLHttpRequest(); xhr.open(\"GET\", urlForOptions); xhr.onload = function() { if (xhr.status === 200) { console.log('result ok'); var obj = JSON.parse(xhr.responseText); var selectOptions = obj.options; if (territoryTypeMap[countryCode] === \"STATE\") { console.log('is state2'); while (provinceSelect.options.length) { provinceSelect.remove(0); } provinceSelect.options.add(blankOption); while (stateSelect.options.length) { stateSelect.remove(0); } if (selectOptions) { var i; for (i = 0; i < selectOptions.length; i++) { var newOption = new Option(selectOptions[i].Option, selectOptions[i].OptionValue); stateSelect.options.add(newOption); } ebx_form_setValue(statePrefixedPath, selectOptions[0].OptionValue); ebx_form_setValue(provincePrefixedPath, null); } else { ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, null); } if (currentStateValue) { try { document.getElementById('customStateSelect').value = currentStateValue; ebx_form_setValue(statePrefixedPath, currentStateValue); } catch (err) {} } } else if (territoryTypeMap[countryCode] === \"PROVINCE\") { console.log('is province2'); while (provinceSelect.options.length) { provinceSelect.remove(0); } while (stateSelect.options.length) { stateSelect.remove(0); } stateSelect.options.add(blankOption); if (selectOptions) { var i; for (i = 0; i < selectOptions.length; i++) { var newOption = new Option(selectOptions[i].Option, selectOptions[i].OptionValue); provinceSelect.options.add(newOption); } ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, selectOptions[0].OptionValue); } else { ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, null); } if (currentProvinceValue) { try { document.getElementById('customProvinceSelect').value = currentProvinceValue; ebx_form_setValue(provincePrefixedPath, currentProvinceValue); } catch (err) {} } } else { console.log('is none2'); while (provinceSelect.options.length) { provinceSelect.remove(0); } provinceSelect.options.add(blankOption); while (stateSelect.options.length) { stateSelect.remove(0); } stateSelect.options.add(blankOption); ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, null); } } else { console.log('result error'); while (provinceSelect.options.length) { provinceSelect.remove(0); } provinceSelect.options.add(blankOption); while (stateSelect.options.length) { stateSelect.remove(0); } stateSelect.options.add(blankOption); ebx_form_setValue(statePrefixedPath, null); ebx_form_setValue(provincePrefixedPath, null); try { ebx_form_setValue(statePrefixedPath, currentStateValue); ebx_form_setValue(provincePrefixedPath, currentProvinceValue); } catch (err) {} } }; xhr.send();");
		writer.addJS("}");
