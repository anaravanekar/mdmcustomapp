...
*/
		...
		writer.add("</td>");
		writer.add("<td colspan=\"1\" style=\"padding-left:5px;" + CELL_STYLE_LEFT + "\">");
		writer.add("<div id='provinceLocalCustomDiv'><select id=\"customProvinceLocalSelect\" onChange=\"var plv=null;if(this.value){plv=this.value;}ebx_form_setValue('"+writer.getPrefixedPath(_ProvinceLocalLanguage).format()+"',plv)\"><option value=\"\">[not defined]</option></select></div>");writer.add("<div id='provinceLocalStandardDiv' style=\"display:none;\">");writer.addWidget(_ProvinceLocalLanguage);writer.add("</div>");
		writer.add("</td>");
		writer.add("</tr>");
