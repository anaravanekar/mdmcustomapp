		...
		if(!isSortByEmpty()) {
			...
			while (sbit.hasNext()) {
				...
				if(null == sbsb) {
					...
					sbsb.append(ORDER_BY).append(pair.getKey()).append(SPACE).append(pair.getValue().value());
				} else {
					sbsb.append(COMMA).append(pair.getKey()).append(SPACE).append(pair.getValue().value());
				}
			}
		}
